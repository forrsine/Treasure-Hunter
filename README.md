...
截至 2026 年 8 月 29 日，项目包含 **254 个自研 C# 脚本**，共 **61,174 个物理代码行**，其中非空行 **53,460 行**。其中 Unity 客户端运行时、Editor 工具及测试共 228 个脚本，.NET 服务端共 26 个源文件。

| 代码范围 | C# 脚本数 | 物理行数 | 非空行数 |
| --- | ---: | ---: | ---: |
| Unity 客户端运行时代码 | 185 | 41,685 | 36,155 |
| Editor 工具与 EditMode 测试 | 43 | 15,981 | 14,254 |
| .NET 服务端代码 | 26 | 3,508 | 3,051 |
| **合计** | **254** | **61,174** | **53,460** |

> 统计范围为 `Assets/Script`、`Assets/Editor` 和 `TreasureHunter.Server` 中的 `.cs` 文件；排除了原始美术素材附带脚本、第三方代码，以及 `Library`、`Temp`、`obj`、`bin` 等缓存或生成目录。物理行数包含代码、注释和空行，非空行数用于辅助说明项目的实际文本规模。
