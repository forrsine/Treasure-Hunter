...
public class PlayerCo : MonoBehaviour, FighterInterface
{
    ...
    private bool CanStartRoll()
    {
        ...
        if (isAttacking || IsRolling || !Input.GetKeyDown(KeyCode.Mouse1))
        {
            return false;
        }
        ...
        // 还必须有移动方向，否则原地右键不翻滚。
        bool hasMoveInput =
            Mathf.Abs(InputCo.Instance.Xinput) > 0.1f ||
            Mathf.Abs(InputCo.Instance.Yinput) > 0.1f;
    ...
    {
        // 从 InputCo 读取已经统一处理过的输入。
        float inputX = InputCo.Instance.Xinput;
        float inputY = InputCo.Instance.Yinput;
    ...
    private void CheckAttackInput()
    {
        ...
        if (!InputCo.Instance.leftMouseDown)
        {
            return;
        }
    ...
    /// </summary>
    private void RecalculateMaxHp(bool fillCurrentHp)
    {
    ...
    /// 从配置读取指定等级升到下一级所需经验。
    /// </summary>
    private int GetNextExpForLevel(int level)
    {
        if (GameConfig.instance == null)
        {
    ...
    /// 通知升级面板：待选择升级次数发生了变化。
    /// </summary>
    private void NotifyPendingUpgradeSelectionsChanged()
    {
        // 通知升级面板：还有几次升级选择没有完成。
