...
public class VaultHudCo : MonoBehaviour
{
    ...
    [Header("References")]
    [Tooltip("可不填。为空时会自动使用运行时上下文里的当前金库。")]
    ...
    private void OnEnable()
    {
        ...
        GameplayRuntime.Instance.CurrentVaultChanged += HandleCurrentVaultChanged;
    ...
    private void HandleCurrentVaultChanged(BoxCo changedVault)
    {
        vault = changedVault;
        Refresh(true);
    }
    ...
    private void ResolveVault()
    {
        ...
        // 优先使用运行时上下文；如果它还没准备好，再用 FindObjectOfType 兜底查找。
        if (GameplayRuntime.Instance.CurrentVault != null)
        {
            vault = GameplayRuntime.Instance.CurrentVault;
            return;
        }
